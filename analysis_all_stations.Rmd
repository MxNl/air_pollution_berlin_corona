---
title: "Air Pollution in Berlin, all period, all stations"
...
Add measurement periods for each station
```{r}
template_url_detailed_information <- "https://luftdaten.berlin.de/station/mc<stationid>#station-info"
```

```{r}
urls_detailed_information <- stations %>%
  select(station_id) %>%
  mutate(url = template_url_detailed_information) %>%
  mutate(url = str_replace(url, "<stationid>", station_id))
```

```{r}
period_start <- urls_detailed_information %>%
  pull(url) %>%
  map(possibly(cus_fun_scrape_measurement_begin, NA))

stations <- stations %>%
  mutate(period_start = unlist(period_start)) %>%
  mutate(period_start = dmy(period_start)) %>%
  select(-description)
```

Remove stations without a value for period_start
```{r}
stations <- stations %>%
  drop_na(period_start)
```

Drop stations at "Stadtrand"
```{r}
stations <- stations %>%
  filter(type != "Stadtrand")
```

```{r}
n_colours <- stations %>% nrow()

stations %>% 
  mutate(period_end = Sys.Date()) %>% 
  pivot_longer(cols = contains("period")) %>% 
  # mutate(test = as.POSIXct())
  ggplot(aes(value, station)) +
  geom_line(size = 5,
            colour = "red",
            alpha = .6) +
  scale_color_manual(values = hues::iwanthue(n_colours))
```
...
urls_to_fetch <- map2_dfr(period_starting_dates,
  period_ending_dates,
  .f = ~ cus_fun_fill_dates_in_url(
    pull(filter(download_urls, station_id == 115), url),
    .x,
    .y
  )
)
